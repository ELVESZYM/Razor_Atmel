...
void UserApp1Initialize(void)
{
 ...
 /* Configure ANT for this application */
  ...
  /* Configure ANT for this application */
  sAntSlaveDate.AntChannel          = ANT_CHANNEL_SLAVE;
  sAntSlaveDate.AntChannelType      = ANT_CHANNEL_TYPE_SLAVE;
  sAntSlaveDate.AntChannelPeriodLo  = ANT_CHANNEL_PERIOD_LO_USERAPP;
  sAntSlaveDate.AntChannelPeriodHi  = ANT_CHANNEL_PERIOD_HI_USERAPP;
  
  sAntSlaveDate.AntDeviceIdLo       = ANT_DerEVICEID_LO_USERAPP;
  sAntSlaveDate.AntDeviceIdHi       = ANT_DEVICEID_HI_USERAPP;
  sAntSlaveDate.AntDeviceType       = ANT_DEVICE_TYPE_USERAPP;
  sAntSlaveDate.AntTransmissionType = ANT_TRANSMISSION_TYPE_USERAPP;
  sAntSlaveDate.AntFrequency        = ANT_FREQUENCY_USERAPP;
  sAntSlaveDate.AntTxPower          = ANT_TX_POWER_USERAPP;

  sAntSlaveDate.AntNetwork = ANT_NETWORK_DEFAULT;
  
  for(u8 i = 0; i < ANT_NETWORK_NUMBER_BYTES; i++)
  {
    sAntSetupData.AntNetworkKey[i] = ANT_DEFAULT_NETWORK_KEY;
    sAntSlaveDate.AntNetworkKey[i] = ANT_DEFAULT_NETWORK_KEY;
  }
